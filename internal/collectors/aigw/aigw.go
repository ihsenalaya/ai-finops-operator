...
func (c *Collector) Collect(ctx context.Context, _ time.Duration) ([]collectors.UsageSample, error) {
	...
	// Explicit LegacyValidation: the newer prometheus/common requires an
	// explicit scheme (the zero-value TextParser{} has an unset scheme and
	// panics on the first metric name it validates). The gateway's metric
	// names are ASCII/legacy, so LegacyValidation matches prior behavior.
